 ...
 */
public class LegacyBreakFragment extends LegacyActionFragment implements NamedFragment, SyncCallback {
	...
	private void initClock() {
		String employeeStatus = UpdateController.INSTANCE.statusWrapper.currentStatus.employeeStatus;
		if (BreakActivity.ON_BREAK.equals(employeeStatus)) {
	...
	private void onShortBreak(View v) {
		...
		setEmployeeStatus(ON_BREAK, activity, false);
		// ((Cache) getActivity()).updateStatusTitle();
		startClock(SHORT_BREAK_TIME);
		((Cache) activity).update();
	}
	...
	private void onLunchBreak(View v) {
		...
		setEmployeeStatus(AT_LUNCH, activity, false);
		// ((Cache) getActivity()).updateStatusTitle();
		startClock(LONG_BREAK_TIME);
		((Cache) activity).update();
	}
	...
	public void updateold() {
		L.out("BreakFragment update: " + view);
		if (view == null)
			return;
		...
		if (user.getEmployeeStatus().equals(LegacyBreakFragment.AVAILABLE)) {
	...
	public void update() {
		...
		if (lastStatus != null && lastStatus.equals(employeeStatus))
			return;
		lastStatus = employeeStatus;
		if (statusWrapper == null
				|| statusWrapper.currentStatus.employeeStatus == null
				|| statusWrapper.currentStatus.employeeStatus.equals(LegacyBreakFragment.AVAILABLE)) {
			takeBreakLayout.setVisibility(View.VISIBLE);
			finishBreakLayout.setVisibility(View.GONE);
			// completeButton.setVisibility(View.INVISIBLE);
			// // completeButton.setText(SELF_TASK);
