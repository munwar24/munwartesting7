...
public enum UpdateController implements SyncCallback {
	...
	public synchronized boolean staticLoad() {
		...
		if (selectLocations == null)
			selectLocations = new SelectLocations();
	...
	public void callback(GJon gJon, String payloadName) {
		...
		// if (gJon instanceof GetActionStatus) {
		// getActionStatus = (GetActionStatus) gJon;
		// actionId = getActionStatus.getActionId();
		// L.out("action actionId: " + actionId);
		// } else if (gJon instanceof GetActorStatus) {
		// getActorStatus = (GetActorStatus) gJon;
		// actionId = getActorStatus.getActionId();
		// L.out("actor actionId: " + actionId);
		// }
		//
		// L.out("actionId: " + actionId);
		// L.out("ERROR: fix logic here!");
		// if (actionId != null) {
		// GetActionStatus temp = getActionStatusHashtable.get(actionId);
		// if (temp == null) {
		// // if (!actionId.startsWith("L")) {
		// // L.out("getting action from server for: " + actionId);
		// // getActionStatus = Flow.getFlow().getActionStatus(actionId);
		// // }
		// L.out("getActionStatus: " + getActionStatus);
		// L.out("getActionStatus json: ");
		// PrettyPrint.prettyPrint(getActionStatus.getJson(), true);
		// getActionStatusHashtable.put(actionId, getActionStatus);
		//
		// getActionHistory.addTarget(getActionStatus.getTarget(0));
		//
		// // doCallback(getActionStatus,
		// // FlowRestService.GET_ACTION_STATUS);
		// } else {
		// L.out("in hash table");
		// getActionStatus = temp;
		// // doCallback(getActionStatus,
		// // FlowRestService.GET_ACTION_STATUS);
		// }
		// } else {
		// L.out("clearing getActionStatus:");
		// getActionStatus = null;
		// }
		// L.out("getActionStatus: " + getActionStatus);
		// // doCallback(getActionStatus, FlowRestService.GET_ACTION_STATUS);
		// doCallback(getActorStatus, FlowRestService.GET_ACTOR_STATUS);
