...
public class FlowRestService {
	...
	public static final String BEACON_SAMPLE = "BeaconSample";
	...
	// public static String SERVER = "https://hrctest.crothall.com/flow/";
	...
	private String getContent(String flowCallName, GetActionStatus getActionStatus, boolean create) {
		...
		L.out("functionalAreaType_id" + getActionStatus.getFunctionalAreaTypeId());

		String note = "";
		if (getActionStatus.getNotes() != null)
			note = getActionStatus.getNotes().replace("\n", " ");
		// L.out("note: " + note.length() + " #" + note + "#");
		// L.out("test now date: " + toNowDate());
	...
	private String addScheduleDate(GetActionStatus getActionStatus) {
		...
		if (UpdateController.selectClassTypesByFacilityId.haveScheduleDate(actionClassType_id))
	...
	private String getStart(GetActionStatus getActionStatus) {
		L.out("getActionStatus.getStartId(): " + getActionStatus.getStartId());
	...
	private String getContent(String flowCallName, String arg1, String arg2) {
		...
		if (flowCallName.equals(GET_ACTOR_STATUS)) {
			...
			content = "{\"data\":{\"facility_id\": \""
					...
					+ "\"}";
			PrettyPrint.prettyPrint(content, true);
			// if (Flow.wantCallBack)
			// UpdateController.INSTANCE.doCallback(new
			// PlainString("selectLocations: "
			// + PrettyPrint.formatPrint(content)),
			// UpdateController.PLAIN_STRING);
			...
			content = "{\"call\":{\"id\":2,\"name\":\"@action.selectClassTypesByFacilityHirNodeId\",\"data\":{\"facilityHirNode_id\": \""
					+ arg1
					+ "\"}},"
					+ "\"target\":\"iiCall\",\"authorization\":\""
					+ Login.authorization
			...
			content = "{\"call\":{\"id\":4,\"name\":\"@common.imMessageUpdate\",\"data\":{"
					...
					// + "\"send\":{"
					+ getItem("target", "DISPATCH", false)
					// + getItem("to", "kim", true)
					// + getItem("by", Login.userName, true)
					// // + "},"
					// + getItem("status", "new", false)
					+ "}},"
					+ "\"target\":\"iiCall\",\"authorization\":\""
					+ Login.authorization
